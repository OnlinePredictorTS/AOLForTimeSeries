...
class RevisionExperimentSetting2(Problem):
	"""
	Description: Simulates an autoregressive moving-average time-series.
	"""

	compatibles = set(['RevisionExperimentSetting2-v0', 'TimeSeries'])
	...
	def initialize(self, p=5, q=2, n = 10, d=1, noise_list = None, c=0, noise_magnitude=0.1, noise_distribution = 'normal'):
		...
		self.phi1 = [random.uniform(generate_key(), shape = (10, 10), minval = -0.025, maxval = 0.025),
		...
		self.phi2 = [random.uniform(generate_key(), shape = (10, 10), minval = -0.025, maxval = 0.025),
			  random.uniform(generate_key(), shape = (10, 10), minval = -0.025, maxval = 0.025),
			  random.uniform(generate_key(), shape = (10, 10), minval = -0.025, maxval = 0.025),
			  random.uniform(generate_key(), shape = (10, 10), minval = -0.025, maxval = 0.025),
			  random.uniform(generate_key(), shape = (10, 10), minval = -0.025, maxval = 0.025),
		...
		for i in range(len(self.phi1)):
			u, s, vh = np.linalg.svd(self.phi1[i])
			parts.append((u, s, vh))
			s_sum += np.sum(s)
		
		if(s_sum > 1):
			for i in range(len(self.phi1)):
				self.phi1[i] = (parts[i][0] * (parts[i][1] / s_sum)) @ parts[i][2]

		parts = []
		s_sum = 0.0
		for i in range(len(self.phi2)):
			u, s, vh = np.linalg.svd(self.phi2[i])
			parts.append((u, s, vh))
			s_sum += np.sum(s)
		
		if(s_sum > 1):
			for i in range(len(self.phi2)):
				self.phi2[i] = (parts[i][0] * (parts[i][1] / s_sum)) @ parts[i][2]
		...
		self.phi1 = np.array(self.phi1)
		self.phi2 = np.array(self.phi2)
		...
		def _step(x, noise, eps):
			...
			alpha = self._alpha()
			x_ar = np.dot(alpha[0], delta_x_d[0])
			for i in range(1, len(alpha)):
				x_ar += np.dot(alpha[i], delta_x_d[i])
	...
	def _alpha(self):
		return (self.phi1 * (self.T / 10000.0) + self.phi2 * (1 - self.T / 10000.0))
